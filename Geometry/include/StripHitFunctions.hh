...
double calclateLocalFromGlobalPostion(const TVector3& hitPosition, int layer,
                                      const fc::DetectorGeometry &);
double calculateStripFromLocalPosition(double localHitPosition, int layer,
                                       const fc::DetectorGeometry &);


// Calculate strip position from cluster
double calculateStripHitPositionFromCluster(int initialStrip,
        const std::vector<int> & stripAdcs);


// From strips number position to global Hit position
double calculateLocalFromStripPosition(double stripHitPosition, int layer,
                                       const fc::DetectorGeometry &);
const TVector3 calculateGlobalFromLocalPosition(double, int,
        const fc::DetectorGeometry &);
